...
def login():
	...
	token = _generate_token({"sub": str(user.get("_id")), "email": user["email"], "role": user["role"], "iat": now, "exp": exp})

	return {"token": token, "role": user["role"], "expires_in": Config.JWT_EXPIRES_IN}, 200
